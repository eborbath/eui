...
```{r}
subset = terms[!terms$number & !terms$nonalpha & terms$characters>2 & terms$termfreq>=25 & terms$reldocfreq<.9, ]
nrow(subset)
head(subset, n=10)
```
...
topmod.plot.topic(m, 4, time_var = meta$date, category_var = meta$medium2, date_interval = "day")
topmod.plot.topic(m, 1, time_var = meta$date, category_var = meta$medium2, date_interval = "day")
```
...
Using the package `vanattevedt/topicbrowser`, you can also create a nice visual browser for inspecting topics. See for example this [analysis of the state of the union](http://rpubs.com/vanatteveldt/topicbrowser).

```{r}
library(topicbrowser)
output = createTopicBrowser(m, t$lemma, t$aid, words=t$word, meta=meta)
````
